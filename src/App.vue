<template>
  <v-app id="inspire">
    <v-navigation-drawer app :mini-variant.sync="mini" mini-variant-width="90">
      ...
      <v-list-item>
        ...
        <h5 class="mb-2" v-if="!mini">Borderless Lab 365</h5>
          <v-list-item-title class="title">
            <p>{{ $route.params.experiment_name }}
      ...
      <v-list dense nav>
        <v-list-item link @click="openManual" v-if="$route.name == 'experiment'">
          <v-list-item-content>
            <v-list-item-title>Lab Manual</v-list-item-title>
          </v-list-item-content>
        </v-list-item>
        <v-list-item link @click="setupDialog = true" v-if="$route.name == 'experiment'">
          <v-list-item-content>
          ...
          <v-list-item-content>
            <v-list-item-title>
              <v-btn dark :block="!mini" color="info" class="mt-4">
                ...
                {{ parseInt(kick_time / 60) }} <span v-if="!mini">mins left</span>
              </v-btn>
            </v-list-item-title>
          </v-list-item-content>
        </v-list-item>
      </v-list>
        <v-btn x-small class="ml-4 my-4" dark color="info" @click="feedbackDialog = true"
          >Feedback</v-btn
        >
        <v-btn v-if="$cookies.isKey('username')" x-small class="ml-4 my-4" dark color="error" @click="goTo('/logout')">
          Logout
        </v-btn>
        <v-btn v-else x-small class="ml-4 my-4" @click="goTo('/login')">Login</v-btn>
        <br>
    ...
    <!-- Setup Dialog -->
    <v-dialog v-model="setupDialog" width="600" persistent v-if="$route.name == 'experiment'">
      <v-card>
    ...
    <v-dialog v-model="feedbackDialog" width="600" persistent>
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" text @click="feedbackDialog = false"> Quit</v-btn>
...
export default {
  ...
  methods: {
    ...
    openManual(){
      window.open(this.$store.state.experiments[this.$route.params.experiment_name].labManual);
    },
    submitFeedback(){
      const url =
      ...
      const data = {
        text:
          ...
          "Experiment: " + this.$route.params.experiment_name + "\n" +
          "Set: " + this.$cookies.get('equipment_id') + "\n" +
